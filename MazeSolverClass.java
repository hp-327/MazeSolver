...
public class MazeSolverClass
   {
    ...
    public MazeSolverClass( int rowHeight, int colWidth,
       ...
       diagnosticDisplayTime = 0.0;
       diagnosticRunFlag = false;
      ...
      maze = new char[numRows][numCols];
       ...
       for (rowIndex = 0; rowIndex < numRows ; rowIndex++)
       {
    	   ...
    	   {
    		   maze[rowIndex][colIndex]= inMaze[rowIndex][colIndex];
    	   }
       }
    ...
    public boolean solveMaze()
       {
    	...
    	int direction = getInitialDirection();
    	LocationData currentRunPos = new LocationData(direction, startY, startX); 
    	LocationData startPos = new LocationData(direction, startY, startX);
    	currentRunPos = setFirstPosition(startPos, TRACK_MARKER);
    	if (!solveMazeHelper(currentRunPos))
    	{
    		startPos = setFirstPosition(startPos, BACKTRACK_MARKER);
    ...
    public boolean solveMazeHelper( LocationData currentLoc )
       {       
        LocationData tempLoc = new LocationData(currentLoc);
        if (isExitAhead(tempLoc))
        {
        	moveToExit(tempLoc);
        	diagnosticDump();
        	return true;
        }
        if (!(isWallAhead( tempLoc)))
        {
        	tempLoc = moveToNextCell(tempLoc);
        	diagnosticDump();
        }
        
        tempLoc = turnToLeft(tempLoc);
        if (!(isWallAhead( tempLoc)))
        {
        	if (solveMazeHelper(tempLoc))
        	{
        		return true;
        	}
        }
        tempLoc = turnToRight(tempLoc);
        if (!(isWallAhead(tempLoc)))
        {
        	if (solveMazeHelper(tempLoc))
        	{
